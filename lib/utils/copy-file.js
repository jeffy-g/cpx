...
/**
 * @typedef {import("stream")} TStream
 * @typedef {import("stream").Writable} TWritable
*/
/**
 ...
 * @param {Required<TCpxOptions>["transform"]} transforms - Factory functions for transform streams.
...
function copyFileContent(source, output, transforms) {
    return new Promise((resolve, reject) => {
        ...
        function cleanup(err) {
            try {
                for (const s of /** @type {TWritable[]} */ (streams)) {
 ...
 * @param {string} source - A path of the source file.
 * @param {string} output - A path of the destination file.
 * @param {Required<Pick<TCpxOptions, "update" | "transform" | "preserve">>} options - Options.
 * @returns {Promise<void>} The promise which will go fulfilled after done.
 */
