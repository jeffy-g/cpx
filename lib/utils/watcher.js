...
const cf = require("./copy-file");
const rf = require("./remove-file");
const debug = d("cpx");
const Minimatch = m.Minimatch;
const { normalizePath } = np;
const { copyFile } = cf;
const { removeFile } = rf;
/**
 * @typedef TWalkDirectoryEntry description
 * @prop {string} path description
 * @prop {Map<string, fs.Stats>} files description
 *
 * @typedef {(() => void) & { clear(): void } & { flush(): void }} TDebounced description
 */
/**
 * @param {string} base
 * @param {boolean | undefined} dereference
 * @param {(path: string, files: Map<string, fs.Stats>) => Promise<void>} callback
 * @returns
 */
const walkDirectories = async function (base, dereference, callback) {
    /** @type {TWalkDirectoryEntry[]} */
    const stack = [];
...
class Watcher extends EventEmitter {
    ...
    open() {
        debug("Watcher#open");
        this.close();
        this.trigger = debounce(this.onTrigger.bind(this), WATCH_DELAY);
    ...
    addDirectory(dirRoot) {
        ...
        return walkDirectories(dirRoot, this.dereference,
        /** @type {(this: Watcher, dirPath: string, files: Map<string, fs.Stats>) => Promise<void>} */
        async (dirPath, files) => {
            ...
            const watcher = fs
                ...
                debug("Watcher~watcher onchange", type, dirPath, filename);
                if (filename == null) {
                    return;
                }
                const sourcePath = normalizePath(path.join(dirPath, filename.toString()));
                fs.stat(sourcePath).then(stat => {
     ...
     */
    removeDirectory(dir) {
        debug("Watcher#removeDirectory", dir);
        const stack = [dir];
        const eventStack = [];
        while (stack.length > 0) {
    ...
    onAdded(sourcePath) {
        ...
        if (this.ready) {
            ...
            this.initialCopyCount += 1;
            this.copy(normalizedPath).then(this.onDoneInitialCopy, this.onDoneInitialCopy);
        }
    }
    /**
    ...
    onTrigger() {
        ...
        (
        /** @type {(this: Watcher) => Promise<void>} */
        async () => {
    ...
    enqueueAdd(sourcePath) {
        ...
        // remove -> change
        // change -> change
        this.queue.set(sourcePath, kind == null || kind === "add" ? "add" : "change");
    ...
    async copy(sourcePath) {
        ...
        if (outputPath !== sourcePath) {
            await copyFile(sourcePath, outputPath, /** @type {Parameters<typeof copyFile>[2]} */ (this));
